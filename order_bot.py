...
MANUAL_ADDRESS_BTN = "✍️ O'zim yozaman"
...
async def phone_given(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    context.user_data["manual_address_mode"] = False
    await update.message.reply_text(
        "Yetkazib berish manzili qanday bo'lsin?\n"
        "📍 Lokatsiya yuborsangiz eng aniq bo'ladi, yoki o'zingiz yozing.",
...
async def address_given(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if message.location:
        ...
        address_text = f"📍 Lokatsiya: https://maps.google.com/?q={lat},{lng}"
    ...
    if text == BACK_BTN:
        ...
        return PHONE
    if text == MANUAL_ADDRESS_BTN:
        context.user_data["manual_address_mode"] = True
        await update.message.reply_text(
            "Manzilingizni yozing (shahar/tuman, ko'cha, uy):",
            reply_markup=nav_keyboard(),
        )
        return ADDRESS

    if context.user_data.get("manual_address_mode"):
        context.user_data["address"] = text
        return await finalize_order(update, context)

    await update.message.reply_text(
        "Iltimos, \"📍 Lokatsiya yuborish\" tugmasini bosing yoki "
        "\"✍️ O'zim yozaman\" ni tanlang.",
        reply_markup=address_keyboard(),
    )
    return ADDRESS
...
def main():
    ...
    conv_handler = ConversationHandler(
        ...
        states={
            ...
            ADDRESS: [MessageHandler((filters.TEXT | filters.LOCATION) & ~filters.COMMAND, address_given)],
        },
        fallbacks=[
